...
const DEFAULT_USER_NAME = "小崔夫妇";
const DEFAULT_USER_AVATAR_PATH = "/xiaocui.jpeg";
...
function resolveUserAvatarUrl(basePath?: string): string {
  const base = basePath?.trim() ? basePath.replace(/\/$/, "") : "";
  return base ? `${base}${DEFAULT_USER_AVATAR_PATH}` : DEFAULT_USER_AVATAR_PATH;
}
...
function renderAvatar(
  ...
  if (normalized === "user") {
    return html`<img
      ...
      src="${resolveUserAvatarUrl(basePath)}"
